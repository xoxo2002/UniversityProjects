        ...
        if (area > 40): 
            ...
            try:
                ...
                if (predicted_label == "Non-defects") or (predicted_label == "Notch"):
                    ...
                    #Textbox and text
                    (text_w, text_h), _ = cv2.getTextSize(predicted_label, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
                    text_x = x1 - 2
                    text_y = y1 - 9
                    #if over top border
                    if text_y - text_h < 0:
                        text_y = text_y + h + 28
                        cv2.rectangle(defect_img, (x1-5, y1+h+5), (x1 + text_w, y1+h+5+20), (255,255,255), -1)
                    #if over right border
                    elif text_x + text_w > defect_img.shape[1]:
                        cv2.rectangle(defect_img, (x1 + w + 5 - text_w - 2, y1 - 20), (x1 + w + 5, y1-5), (255,255,255), -1)
                        text_x = x1 + w + 5 - text_w
                    else: 
                        cv2.rectangle(defect_img, (x1-5, y1 - 20), (x1 + text_w, y1-5), (255,255,255), -1)
                    cv2.putText(defect_img, predicted_label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0,0,0), 1)
                    continue
                else:  
